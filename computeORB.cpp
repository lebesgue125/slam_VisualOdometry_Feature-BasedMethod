...
int main(int argc, char **argv) {
    ...
    cout << "keypoints1: " << keypoints.size() << endl;
    ...
    // compute angle for each keypoint2
    computeAngle(second_image, keypoints2);

// plot the keypoints2222222222222222222
    cv::Mat image_show2;
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    ...
    for (auto &kp : keypoints) {
        ...
        kp.angle=atan(m10/m01)*180.0f/pi;
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    ...
    int count=0;
    for (auto &kp: keypoints) {
        cout<<"count   "<<count++<<":"<<kp.pt.x<<"----------"<< kp.pt.y<<endl;
        ...
        for (int i = 0; i < 256; i++) {
            ...
            // rotate with theta
            float cos_theta=cos(kp.angle* pi / 180.0);
            float sin_theta=sin(kp.angle* pi / 180.0);
            ...
            cv::Point2f qq = cv::Point2f(cos_theta * q.x - sin_theta * q.y, sin_theta * q.x + cos_theta * q.y)
                             + kp.pt;
            //cout<<"pp&qq :"<<pp.x<<","<<pp.y<<" and "<<qq.x<<","<< qq.y<<endl;
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
        ...
        for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
            ...
            int distance = 0;
            for (int i=0;i<256;i++){
                distance+= desc1[i1][i]==desc2[i2][i]?0:1;
            }
